...
class PostgresBuilder(BaseBuilder):
    ...
    def get(self):
        return [Dynamic(index) for index in self._get_connection().execute_select(self._compile_select())]

    def lists(self, columns):
        return Response(self._get_connection().execute_select(self._compile_select())).tolist(columns)

    def pluck(self, key, value):
        return Response(self._get_connection().execute_select(self._compile_select())).pluck(key, value)

    def data(self):
        return Response(self._get_connection().execute_select(self._compile_select())).data()

    def response(self):
        return Response(self._get_connection().execute_select(self._compile_select()))
        ...
        if length == 1 and isinstance(args[0], dict):
            ...
            if args[1] in self.operators:
                if args[1] == '=':
                    ...
                    self.__orwhere__.append((args[0], '::text ' + args[1], args[2]))
                else:
