...
class ModelDemo1(BaseModel):
    ...
    __tablename__ = 'lh_test_copy1'  # 表名
...
if __name__ == '__main__':
    ...
    #     ModelDemo().where('id', 2).update({'name': "5555"})
    #     ModelDemo().where('id', id).update({'name': "444","token_name":'111'})
    #
    ...
    data = ModelDemo().where({'id': 1, 'name': 'hehe', }).where('id', '>', 0).get()
    ...
    data = ModelDemo().select(
        "to_char(to_timestamp(create_time),'yyyy-MM-dd HH24-MI-SS')as time").get()  # select from_unixtime(create_time) as time from lh_test
    ...
    data = ModelDemo().select('count(*) as num', 'name').groupby('name').having('count(*)', 2).get()
    # select name from lh_test having left(name, 4) = 'haha'
    data = ModelDemo().select('name').groupby('name').having('left(name, 4)', '=', 'haha').get()
    ...
    data = ModelDemo('a').select('a.name', 'a.id').where('id', 62).get()
